...
A small `right.step` result cache was also removed. The smoke runs showed no
useful hit rate, and the extra bookkeeping made the hot path harder to reason
about.
...
So the data structure fixed a real asymptotic waste, but it did not remove the
...
cost, the next likely targets are `right.step` volume in the span path and the
generic set-trie fallback for higher-arity rules.
