...
- Added bounded recursive fast paths with iterative fallback, retaining low
  constants on ordinary trees and stack safety on depth-20,000 trees.
...
- Deferred binary successor construction until a second item is requested and
  represented pending expansion with a cursor rather than a per-state vector.
- Added an adaptive paged state-stream table that retains the direct map for
  small traversals and compacts hash buckets after 4,096 touched states.
- Adopted packed-term-arena 0.1.3's allocation-free leaf/slice insertion in
  recursive and iterative tree materialization.
